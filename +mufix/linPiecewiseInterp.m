function [yhat, cfg, figs] = linPiecewiseInterp(y,fs,tseg,cfg_)
...
for ii = 1:nTseg
  ...
  tframe  = idx_tseg(ii,1):idx_tseg(ii,2);
  yhat(tframe) = linspace(y(tframe(1)-1), y(tframe(end)+1), length(tframe));

  cfg.tframe{ii}  = tframe;
  % cfg.yhat{ii}    = yhat(tframe);

  % % cropidx = [idx_tseg(ii,1) + idx_crop_pre, idx_tseg(ii,2) + idx_crop_post];
  % % yhat(cropidx) = linspace(y(idx_tseg(ii,1) - 10), y(idx_tseg(ii,2) + 10), length(cropidx));
