function [carrier,cfg] = simulate_crosstalk(tdtmat, carrier, fs_mod)
...
%   Detailed explanation goes here
    ...
    cfg = struct();
    cfg.xtalk_on  = stim_on;
    cfg.xtalk_off = stim_off;

  % % function reboundx = get_rebound()
  % %     reboundx(1) = -1.0;
  % %     reboundx(2) = -1.5;
  % %     reboundx(3) = -0.5;
  ...
  function reboundx = get_rebound()
      reboundx(1) = ctcenter;
      reboundx(2) = (3*ctcenter)/4;
      reboundx(3) = (2*ctcenter)/3;
      reboundx(4) = ctcenter/2;
      reboundx(5) = ctcenter/3;
      reboundx(6) = ctcenter/4;
      reboundx(7) = ctcenter/8;
      reboundx(8) = -1.5;
      reboundx(9) = -0.5;
      reboundx(10) = ctcenter/8;
      reboundx(11) = ctcenter/4;
      reboundx(12) = ctcenter/3;
      n = round(rebound_s * fs_mod);
      reboundx = interp1(linspace(0,1,12), reboundx, linspace(0,1,n));
  end
